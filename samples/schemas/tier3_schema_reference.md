# Tier 3 Schema Reference

This document provides exhaustive field-by-field documentation for the Tier 3 daily parquet exports.

For an introduction to the dataset, quick start examples, and file layout, see [README_TIER3.md](README_TIER3.md).
...
| `meta` | struct | Entry metadata including timestamps, schema version, session lifecycle, and universe linkage. See [meta](#meta) for full field details |
| `spot_raw` | struct | Raw spot market data from Binance API endpoints including prices, spreads, order book depth, and microstructure metrics. See [spot_raw](#spot_raw) for full field details |
| `futures_raw` | struct | USDT-margined perpetual futures data from Binance FAPI including funding rates, open interest, basis, and trader positioning. Null for symbols without perpetual contracts. See [futures_raw](#futures_raw) for full field details |
| `derived` | struct | Computed composite metrics derived from raw market data. These are NOT raw API responses but calculated values. Most are set at admission from scoring factors; spread_pct and spread_bps are updated by the sampler during the 2-hour session. See [derived](#derived) for full field details |
| `scores` | struct | Internal factor scores used for ranking and admission decisions. Each score is a normalized number from 0 to 100, where the mapping from raw market data to score varies by factor type. These are NOT percentages - they are arbitrary scoring units designed to make factors comparable. See [scores](#scores) for full field details |
| `flags` | struct | Boolean indicators about data quality and scoring behavior. Use these to filter entries with incomplete or degraded data. true = condition met/enabled, false = condition not met/disabled. See [flags](#flags) for full field details |
| `diag` | struct | Diagnostic metadata for troubleshooting and quality assurance. NOT intended as ML features - these track build process health and validation status. See [diag](#diag) for full field details |
| `twitter_sentiment_windows` | struct | X (Twitter) sentiment data for multiple time windows (last_cycle and last_2_cycles). Contains post counts, lexicon sentiment, AI sentiment, engagement metrics, author stats, and more. See [twitter_sentiment_windows](#twitter_sentiment_windows) for full field details |
| `twitter_sentiment_meta` | struct | Metadata about sentiment data capture including source, timestamps, and bucket configuration. See [twitter_sentiment_meta](#twitter_sentiment_meta) for full field details |
| `spot_prices` | list\<struct\> | Time-series price data: Array of 700+ price samples collected every ~10 seconds during the 2-hour tracking session. Use this for backtesting entry/exit timing, calculating MAE/MFE, or analyzing price action patterns. First sample is near session admission (added_ts), last sample is near expiry (expired_ts). See [spot_prices](#spot_prices-list-element) for full field details |

> **Schema Reference:** For the complete annotated schema with detailed explanations for every field, see [`data/schema/ARCHIVE_ENTRY_FULL_SCHEMA.json`](../../data/schema/ARCHIVE_ENTRY_FULL_SCHEMA.json).
...
Entry metadata including timestamps, schema version, and session lifecycle tracking | Field | Type | Description |
|-------|------|-------------|
...
Raw spot market data from Binance API endpoints | Field | Type | Description |
|-------|------|-------------|
...
> **SECTION NOTE:** Computed composite metrics derived from raw market data. These are NOT raw API responses but calculated values. Most are set at admission from scoring factors; spread_pct and spread_bps are updated by the sampler during the 2-hour session | Field | Type | Description |
|-------|------|-------------|
...
> **SECTION NOTE:** Internal factor scores used for ranking and admission decisions. Each score is a normalized number from 0 to 100, where the mapping from raw market data to score varies by factor type. These are NOT percentages - they are arbitrary scoring units designed to make factors comparable. The 'final' score combines individual factors with configurable weights. Use these scores to understand WHY a coin was selected and to build your own filtering/ranking logic | Field | Type | Description |
|-------|------|-------------|
...
> **SECTION NOTE:** Diagnostic metadata for troubleshooting and quality assurance. NOT intended as ML features - these track build process health and validation status. Use these to identify entries with build issues or missing validation | Field | Type | Description |
|-------|------|-------------|
...
Lexicon-based sentiment scoring using term matching | Field | Type | Description |
|-------|------|-------------|
...
Counts of matched sentiment terms by category across all tweets in this cycle | Field | Type | Description |
|-------|------|-------------|
...
Up to 3 most frequently matched terms per category across all tweets in this cycle (arrays of strings) | Field | Type | Description |
|-------|------|-------------|
...
Engagement metrics summed across all tweets in this cycle | Field | Type | Description |
|-------|------|-------------|
...
Statistics about tweet authors in this cycle | Field | Type | Description |
|-------|------|-------------|
...
Content type statistics for tweets in this cycle | Field | Type | Description |
|-------|------|-------------|
...
Recency and activity indicators for this cycle | Field | Type | Description |
|-------|------|-------------|
...
AI model outputs using DistilBERT transformer fine-tuned on crypto Twitter data for binary positive/negative classification | Field | Type | Description |
|-------|------|-------------|
...
| `primary_model` | string | Name/version of the primary sentiment model. Example: 'run8_final'. This model is a DistilBERT transformer fine-tuned on crypto Twitter data for binary positive/negative classification |
| `referee_model` | string/null | Name/version of the referee model used in hybrid mode. Example: 'run7_final'. This model has well-calibrated confidence scores and is used to decide when to override the primary or classify as neutral. NULL in single-model mode |
...
| `prob_mean` | double/null | Mean probability score from the primary model across all scored tweets. Range: 0.0 to 1.0, where >0.5 indicates the model classified sentiment as positive |
...
| `prob_min` | double/null | Minimum probability score among all scored tweets. Indicates the most confidently negative classification |
| `prob_max` | double/null | Maximum probability score among all scored tweets. Indicates the most confidently positive classification |
...
Final hybrid two-model decision metrics. The hybrid system uses a primary model for classification and a referee model for confidence calibration and overrides | Field | Type | Description |
|-------|------|-------------|
...
| `mean_score` | double | Mean of hybrid_score_3class values across all tweets. Each tweet gets -1.0 (negative), 0.0 (neutral), or +1.0 (positive). Range: -1.0 to +1.0. *Note: Sentiment means are typically positive due to structural properties of crypto social discourse; interpret alongside post counts, balance, and silence indicators.* |
...
| `primary_conf_mean` | double/null | Mean confidence score from the primary model across all scored tweets. Range: 0.0 to 1.0. Higher values indicate more confident classifications |
...
Breakdown of how decisions were made in the hybrid system | Field | Type | Description |
|-------|------|-------------|
...
All category counts are **SUMmed** across both cycles | Field | Type | Aggregation | Description |
|-------|------|-------------|-------------|
...
All top_terms arrays are **MERGEd** and deduplicated across both cycles, preserving order from cycle N-1 then N | Field | Type | Aggregation | Description |
|-------|------|-------------|-------------|
...
All content_stats fields are **SUMmed** across both cycles | Field | Type | Aggregation | Description |
|-------|------|-------------|-------------|
...
All decision_sources counts are **SUMmed** across both cycles | Field | Type | Aggregation | Description |
|-------|------|-------------|-------------|
...
Metadata about sentiment data capture. This section contains provenance information about when and how the sentiment data was collected | Field | Type | Description |
|-------|------|-------------|
